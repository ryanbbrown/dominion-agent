...
var ais = {
  ...
  get 'claude-3-5-sonnet-new'() {
    return require('../llmai/dist/claude-3-5-sonnet-new');
  },
  get 'deepseek-chat'() {
    return require('../llmai/dist/deepseek-chat');
  },
  get 'deepseek-reasoner'() {
    return require('../llmai/dist/deepseek-reasoner');
  },
};
...
function enumerateMoves(state) {
  ...
  function enumerateBuyMoves() {
    for (var i = 0; i < state.cardList.length; i += 1) {
      ...
      if (gameCard.count > 0 && state.treasureCount >= gameCard.card.cost && !gameCard.card.excludeFromSupply) {
        moves.push({
          name: 'buy',
          params: {
            card: gameCard.card.name,
...
function checkActionsOver(state) {
  ...
  if (state.state === STATE_LIBRARY_DRAW) {
    ...
    if (isCardType(drawnCard, 'Action')) {
      ...
      state.lastDrawnCard = drawnCard.name;
      return;
    }
    checkActionsOver(state);
    return;
...
function playerCleanUpHand(state, player) {
  // Handle Page upgrades
  for (var i = 0; i < player.inPlay.length; i++) {
    var card = player.inPlay[i];
    if (card.name === 'Page' && card.travellerUpgrade) {
      // Return Page to supply
      var gameCard = state.cardTable[card.name];
      gameCard.count++;
      
      // Gain the upgrade card
      var upgradeCard = state.cardTable[card.travellerUpgrade];
      if (upgradeCard) {
        playerGainCard(state, player, upgradeCard, false, false);
      }
    }
  }

  while (player.inPlay.length > 0) {
    var card = player.inPlay[player.inPlay.length - 1];
    if (card.name !== 'Page' || !card.travellerUpgrade) {
      player.discardPile.push(player.inPlay.pop());
    } else {
      player.inPlay.pop(); // Page is already handled above
    }
  }
...
DominionGame.prototype.shuffleAndDeal = function (playerAiList, seed) {
  ...
  while (kingdomCards.length < 10) {
    ...
    if (list.length > 0) {
      ...
      // Skip the Page upgrade line cards (they can only be included if Page is selected)
      if (['Treasure Hunter', 'Warrior', 'Hero', 'Champion'].includes(card.name)) {
        continue;
      }
      ...
      // If Page is selected, add its entire upgrade line
      if (card.name === 'Page') {
        var upgradeCards = ['Treasure Hunter', 'Warrior', 'Hero', 'Champion'];
        for (var i = 0; i < upgradeCards.length; i++) {
          var upgradeCard = dominion.cardTable[upgradeCards[i]];
          if (upgradeCard) {
            // Add upgrade cards to the supply but don't count them towards the 10 kingdom cards
            addCard(this, upgradeCard);
          }
        }
      }
    }
...
function stateIndexToString(state) {
  switch (state.state) {
    ...
    case STATE_BUY:
      return "buy a card or end their turn";
    case STATE_DISCARD_THEN_DRAW:
    ...
    case STATE_LIBRARY_CHOOSE:
      return `choose whether to keep or set aside ${state.lastDrawnCard}`;
    case STATE_PUT_ON_DECK_FROM_DISCARD:

...
game.printSupplyDetails = function(includeStrategy = false) {
  const [treasureCards, victoryCards, kingdomCards] = getAndSortSupplyGroups();
  const output = `CARD DESCRIPTIONS
...
KINGDOM CARDS
${kingdomCards.map(c => game.printCardDetails(c.card, includeStrategy)).join(includeStrategy ? '\n\n\n' : '\n')}
`;
  console.log(output);
  return output;
}

game.printCardDetails = function(card, includeStrategy) {
  if (typeof card == 'string') {
    card = game.cardList.find(c => c.card.name == card)?.card;
  }
  ...
  if (!includeStrategy || !card.strategy) {

...
export const SYSTEM_PROMPT = `You are nasmith99, the most skilled Dominion player in the world.`;
...
const INSTRUCTIONS_EVALUATE = (moveNum: number, move: string) => `INSTRUCTIONS
The player is considering making move #${moveNum} ${move}.

Do you agree that this is a good move to make? Explain why or why not.

Make sure you consider factors such as
* the current state of the games
* The cards that the player already owns
...
const INSTRUCTIONS_PLAY_AFTER_FEEDBACK = (moves: string[], explanation1: string, explanation2: string) => `INSTRUCTIONS
The player is debating between making one of the following two moves and has provided their reasoning.
* Summarize the current game based on the logs, available kingdom cards, etc...
* List out the pros and cons of each move.
* Then choose the best move and provide a short explanation why.
...
export function makeAI(send: (chatLog: ChatLog, json: boolean) => Promise<string>) {
    return async function chooseMove(dominion: any, state: any, moveList: any, callback: any) {
        async function tryAI(chatRequest: ChatLog, json: boolean, callback: (output: string) => void) {
            ...
            while (--attempts) {
                try {
                    ...
                    const output = await send(chatRequest, json);
                    console.log();
            ...
            throw new Error(`AI unable to successfully respond after ${RETRY_ATTEMPTS} attempts`);
        }

        // There's a small amount of possible information disclosure here,
        // but for the most part the AI should always just play all treasure cards for the buy phase.
        ...
        function isValidMove(choice: number) {
        ...
        await tryAI([
            ...
            if (!parsed.moveExplanation || !parsed.move) {
                throw new Error('AI did not provide a move and explanation');
            }
            const firstChoice = parsed.move;
            const firstExplanation = parsed.moveExplanation;
            isValidMove(firstChoice);

            const instructions = INSTRUCTIONS_EVALUATE(firstChoice, state.moveToString(moveList[firstChoice - 1]));
            console.log();
            ...
            await tryAI([
                ...
                const parsed = JSON.parse(output);
                if (parsed.agree) {
                    callback(null, moveList[firstChoice - 1]);
                } else {
                    if (!parsed.moveExplanation || !parsed.move) {
                        throw new Error('AI did not provide a move and explanation');
                    }
                    isValidMove(parsed.move);

                    const choices = [moveList[firstChoice - 1], moveList[parsed.move - 1]];

                    const instructions = INSTRUCTIONS_PLAY_AFTER_FEEDBACK(choices.map(_ => state.moveToString(_)), firstExplanation, parsed.moveExplanation);
                    console.log();
                    console.log(instructions);
                    ...
                    await tryAI([
                        ...
                        { role: "user", content: getGameRules(state.printSupplyDetails(true)) },
                        { role: "user", content: `GAME LOG\n${(state.printLog() as string[]).slice(-100).join('\n')}` },
                        { role: 'user', content: state.printGameState() },
                        { role: 'user', content: instructions },
                        { role: 'user', content: state.printMoveList(choices) },
                    ], true, async (output) => {
                        const parsed = JSON.parse(output);
                        if (parsed.move != 1 && parsed.move != 2) {
                            throw new Error(`${parsed.move} is not a valid move`);
                        }
                        callback(null, choices[parsed.move - 1]);
                    });
                }
            });
        })
